  ...
  return (
    <div>
      <Button
        ...
        title="Download as a Claw Patrol test fixture"
      >
        {busy ? "Downloading…" : "Download action"}
      </Button>
      {err && <div className="mt-1 text-xs text-danger-500">{err}</div>}
    </div>
  );
}
...
function CaptureStateBadge({ state }: { state: BodyCaptureState }) {
  // "unknown" is the state of rows recorded before capture tracking
  // existed and of plugin-family rows, which never carry one; a badge
  // on every such row would be noise. Fixture export explains itself
  // when it refuses one.
  if (state === "complete" || state === "unknown") return null;
